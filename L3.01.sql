...
JOIN account_demo
USING(district_id);
-- on district_demo.district_id = account_demo.district_id;
...
-- change accountDemo location to "north" if district_id > 3 and "south otherwise
...
CREATE TABLE account_demo (
  ...
  frequency text,
  date int DEFAULT NULL,
  CONSTRAINT FOREIGN KEY (district_id) REFERENCES district_demo(dist_id)
);
...
UPDATE account_demo set location =
  CASE
